...
class PsychologyBot:
    ...
    async def show_main_menu(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        #logger.info(f"[MAIN_MENU] Показываем главное меню пользователю {user_id} (имя: {name})")
        
        try:
            # Сразу показываем инструкцию для записи голосового сообщения
            await update.message.reply_text(
            ...
            await update.message.reply_text(
                ...
                "а продолжительность всего разговора не более 30 минут",
                reply_markup=ReplyKeyboardRemove()
            )
            #logger.info(f"[MAIN_MENU] Инструкция для записи отправлена пользователю {user_id}")
        except Exception as e:
            #logger.error(f"[MAIN_MENU] Ошибка отправки инструкции пользователю {user_id}: {e}")
